...
In the data tidying phase, I gathered the observation from week 1 to 8 under "week" variable and separate all subjects into control and experimental arms. In the final dataset I generated, there are `r nrow(my_tidy_data)`observations and `ncol(my_tidy_data)` columns. Variables includes group, subject id, week and value.
...
```{r}
prop_homicide = homicide_crime %>% 
	nest(city_state, city_total:unsolved) %>%
	mutate(prop = map(.x = data,
												 function(x) {
												 	broom::tidy(prop.test(x$unsolved, x$city_total))
												 	})) %>% 
	unnest() %>% 
